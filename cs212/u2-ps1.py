...
def compile_formula(formula, verbose=False):
    ...
    body = ''.join(tokens)
    modifiers = []
    for word in re.split('([A-Z]+)', formula):
        if word.isupper() and len(word) > 1: modifiers.append(word[0] + '!=0')
    body = ' and '.join(modifiers) + ' and ' + body
    f = 'lambda %s: %s' % (parms, body)
    if verbose: print f
    return eval(f), letters

def compile_formula_2(formula, verbose=False):
    """Compile formula into a function. Also return letters found, as a str,
    in same order as parms of function. The first digit of a multi-digit 
    number can't be 0. So if YOU is a word in the formula, and the function
    is called with Y eqal to 0, the function should return False."""
...
formula = 'YOU == ME**2'
f, p = compile_formula(formula, verbose=True)
compile_formula_2(formula, verbose=True)
print f, p, f(0,0,6,6,3), faster_solve(formula)
